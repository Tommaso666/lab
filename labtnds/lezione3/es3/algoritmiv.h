...
template <typename T> void importo_txt_v(const char *);
...
template <typename T> vector<T> importo_txt_v( char *filename){
...
template <typename T> double min(const vector<T> & y0){
        vector<T> y = y0;
    sort(y.begin(),y.end());
    return y[0];
    
}
template <typename T> double max(const vector<T> & y0){
           vector<T> y = y0;
    sort(y.begin(),y.end());
