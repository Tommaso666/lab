...
class integratore {
    ...
    virtual double integrale(const funzione& f ,double a,double b,double pass) = 0;
    virtual ~integratore() {}; 
    protected:
    double m_a ,m_b;
    unsigned int passi;
...
class middlepoint : private integratore {
    public:
    middlepoint(){};
    double integrale(const funzione& f ,double a,double b,double pass) override{
        double h =(b-a)/pass;
        double somma;
        double calcinx=0;
...
class simson : private integratore {
    public:
    simson(){};
    double integrale(const funzione& f ,double a,double b,double pass) override{
        double h =(b-a)/pass;
...
class trapezi : private integratore {
    public:
    trapezi(){};
    double integrale(const funzione& f, double a, double b, double err) override {
    unsigned int pass = 2; // Numero iniziale di passi
    double myerr = 1e9;    // Errore iniziale molto grande
    double calcinx = 0.5 * (f.val(a) + f.val(b)); // Stima iniziale
    double controllo;
    int noloop=0;

    if (a > b) {
        cout << "Il valore dell'integrale da (" << a << " ; " << b 
             << ") equivale a -integrale da (" << b << " ; " << a 
             << ") controlla se il valore è quello che cercavi" << endl;
        return -integrale(f, b, a, err);
    }

    do {
        controllo = 0.5 * (f.val(a) + f.val(b)); // Reset di controllo
        double h = (b - a) / pass;

        for (unsigned int i = 1; i < pass; i++) {
            controllo += f.val(a + i * h);
        }
        controllo *= h;

        myerr = fabs(controllo - calcinx); // Aggiorna l'errore
        calcinx = controllo;              // Aggiorna il valore dell'integrale
        pass *= 2;                        // Raddoppia i passi
    // cout<< "non sto uscendo "<<noloop<<endl;
    if (noloop>20){break;};
    noloop++;
    } while (myerr > err);
    cout<< "sono uscito dopo "<<noloop <<"passaggi"<<endl;
    // cout<<"figa sono uscito"<<endl;
    return calcinx;
}
